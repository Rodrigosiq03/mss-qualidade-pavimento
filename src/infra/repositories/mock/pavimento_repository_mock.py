...
class PavimentoRepositoryMock(IPavimentoRepository):
  def __init__(self):
    self.segmentos = []
    self.pavimentos = [
      Pavimento(
  ...
  def get_pavimento(self, protocolo_id: int):
    return [segmento for segmento in self.segmentos if segmento.protocolo_id == protocolo_id]
  
  def save_pavimento(self, pavimento: Pavimento) -> Pavimento:
